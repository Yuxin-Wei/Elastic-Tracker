...
class Nodelet : public nodelet::Nodelet {
 private:
  ...
  // NOTE just for debug
  bool debug_ = false;
  bool visibility_visualize_ = true;
  ...
  void visualize_ray_visibility(const Trajectory& traj) {
    std::vector<visualization::RayVisibilitySample> samples;
    if (!visibility_visualize_) {
      return;
    }

    if (trajOptPtr_->visibility_model_ == "ray") {
      const int sample_count = trajOptPtr_->tracking_ps_.size() * 4 / 5;
      const double traj_duration = traj.getTotalDuration();
      samples.reserve(sample_count);
      for (int i = 0; i < sample_count; ++i) {
        const double t = i * trajOptPtr_->tracking_dt_;
        if (t >= traj_duration) {
          break;
        }

        visualization::RayVisibilitySample sample;
        sample.uav_p = traj.getPos(t);
        sample.target_p = trajOptPtr_->tracking_ps_[i];
        if (!trajOptPtr_->evaluateRayOpticalDepth(
                sample.uav_p, sample.target_p, sample.tau, sample.grad_tau)) {
          continue;
        }
        sample.transmittance = std::exp(-sample.tau);
        Eigen::Vector3d grad_cost;
        trajOptPtr_->grad_cost_visibility_ray(
            sample.uav_p, sample.target_p, grad_cost, sample.cost);
        samples.push_back(sample);
      }
    }

    visPtr_->visualize_ray_visibility(
        samples, trajOptPtr_->visibility_T_safe_, "ray_visibility");
  }
  ...
  void plan_timer_callback(const ros::TimerEvent& event) {
    ...
    if (generate_new_traj_success) {
      ...
      if (generate_new_traj_success && !land_triger_received_) {
        visualize_ray_visibility(traj);
      }
  ...
  void init(ros::NodeHandle& nh) {
    ...
    nh.param("visibility/visualize", visibility_visualize_, true);
    ...
    trajOptPtr_ = std::make_shared<traj_opt::TrajOpt>(nh, gridmapPtr_);
