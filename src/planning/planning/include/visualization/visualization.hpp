...
struct RayVisibilitySample {
  Eigen::Vector3d uav_p;
...
class Visualization {
 ...
 public:
  ...
  template <class TOPIC>
  void visualize_ray_visibility(const std::vector<RayVisibilitySample>& samples,
                                const double T_safe,
                                const TOPIC& topic) {
    auto got = publisher_map_.find(topic);
    if (got == publisher_map_.end()) {
    ...
    visualization_msgs::MarkerArray marker_array;
    visualization_msgs::Marker clear_previous;
    clear_previous.action = visualization_msgs::Marker::DELETEALL;
    marker_array.markers.push_back(clear_previous);

    visualization_msgs::Marker rays;
    rays.header.frame_id = "world";
    rays.header.stamp = ros::Time::now();
    rays.ns = "transmittance_rays";
    rays.id = 0;
    rays.type = visualization_msgs::Marker::LINE_LIST;
    rays.action = visualization_msgs::Marker::ADD;
    setMarkerPose(rays, 0, 0, 0);
    setMarkerScale(rays, 0.04, 0.04, 0.04);
    rays.points.reserve(2 * samples.size());
    rays.colors.reserve(2 * samples.size());

    for (size_t i = 0; i < samples.size(); ++i) {
      const auto& sample = samples[i];
      geometry_msgs::Point uav_point;
      uav_point.x = sample.uav_p.x();
      uav_point.y = sample.uav_p.y();
      uav_point.z = sample.uav_p.z();
      geometry_msgs::Point target_point;
      target_point.x = sample.target_p.x();
      target_point.y = sample.target_p.y();
      target_point.z = sample.target_p.z();
      rays.points.push_back(uav_point);
      rays.points.push_back(target_point);

      std_msgs::ColorRGBA ray_color;
      ray_color.a = 0.9;
      if (sample.transmittance >= T_safe) {
        ray_color.r = 0.1;
        ray_color.g = 0.9;
        ray_color.b = 0.2;
      } else {
        const double ratio = std::max(0.0, std::min(1.0, sample.transmittance / T_safe));
        ray_color.r = 1.0;
        ray_color.g = ratio;
        ray_color.b = 0.0;
      }
      rays.colors.push_back(ray_color);
      rays.colors.push_back(ray_color);

      const double grad_norm = sample.grad_tau.norm();
      if (grad_norm > 1e-8 && sample.transmittance < T_safe) {
        visualization_msgs::Marker descent;
        descent.header = rays.header;
        descent.ns = "descent_direction";
        descent.id = static_cast<int>(i);
        descent.type = visualization_msgs::Marker::ARROW;
        descent.action = visualization_msgs::Marker::ADD;
        setMarkerPose(descent, 0, 0, 0);
        setMarkerScale(descent, 0.04, 0.09, 0.12);
        setMarkerColor(descent, yellow, 0.9);
        descent.points.resize(2);
        descent.points[0] = uav_point;
        const Eigen::Vector3d descent_end =
            sample.uav_p - 0.6 * sample.grad_tau / grad_norm;
        descent.points[1].x = descent_end.x();
        descent.points[1].y = descent_end.y();
        descent.points[1].z = descent_end.z();
        marker_array.markers.push_back(descent);
      }

      visualization_msgs::Marker diagnostics;
      diagnostics.header = rays.header;
      diagnostics.ns = "diagnostics";
      diagnostics.id = static_cast<int>(i);
      diagnostics.type = visualization_msgs::Marker::TEXT_VIEW_FACING;
      diagnostics.action = visualization_msgs::Marker::ADD;
      setMarkerPose(diagnostics, sample.uav_p.x(), sample.uav_p.y(),
                    sample.uav_p.z() + 0.25);
      setMarkerScale(diagnostics, 0.0, 0.0, 0.14);
      setMarkerColor(diagnostics, white, 0.95);
      std::ostringstream text;
      text << std::fixed << std::setprecision(2)
           << "tau " << sample.tau << "  T " << sample.transmittance
           << "\n|grad_tau| " << grad_norm << "  J "
           << std::scientific << std::setprecision(1) << sample.cost;
      diagnostics.text = text.str();
      marker_array.markers.push_back(diagnostics);
    }

    if (!samples.empty()) {
      marker_array.markers.push_back(rays);
    }
    publisher_map_[topic].publish(marker_array);
  }
