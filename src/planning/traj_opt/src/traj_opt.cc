...
TrajOpt::TrajOpt(ros::NodeHandle& nh,
                 const std::shared_ptr<mapping::OccGridMap>& map_ptr)
    : nh_(nh), mapPtr_(map_ptr) {
  // nh.getParam("N", N_);
  nh.getParam("K", K_);
  ...
  nh.param<std::string>("visibility/model", visibility_model_, "ray");
  nh.param("visibility/rho", rhoVisibilityRay_, 10000.0);
  nh.param("visibility/ray_samples", visibility_ray_samples_, 20);
  nh.param("visibility/T_safe", visibility_T_safe_, 0.80);
  nh.param("visibility/occ_max_prob", visibility_occ_max_prob_, 0.95);
  nh.param("visibility/unknown_prob", visibility_unknown_prob_, 0.0);
  nh.param("visibility/eps", visibility_eps_, 1e-6);
  nh.param("visibility/debug", visibility_debug_, false);

  if (visibility_model_ != "legacy" && visibility_model_ != "ray" &&
      visibility_model_ != "none") {
    ROS_WARN_STREAM("Unknown visibility/model '" << visibility_model_
                                                   << "'; using 'ray'.");
    visibility_model_ = "ray";
  }
  if (!std::isfinite(rhoVisibilityRay_) || rhoVisibilityRay_ < 0.0) {
    ROS_WARN("visibility/rho must be finite and nonnegative; using 10000.0.");
    rhoVisibilityRay_ = 10000.0;
  }
  if (visibility_ray_samples_ <= 0) {
    ROS_WARN("visibility/ray_samples must be positive; using 20.");
    visibility_ray_samples_ = 20;
  }
  if (!std::isfinite(visibility_T_safe_) || visibility_T_safe_ <= 0.0 ||
      visibility_T_safe_ > 1.0) {
    ROS_WARN("visibility/T_safe must be in (0, 1]; using 0.80.");
    visibility_T_safe_ = 0.80;
  }
  if (!std::isfinite(visibility_occ_max_prob_) ||
      visibility_occ_max_prob_ < 0.0 || visibility_occ_max_prob_ >= 1.0) {
    ROS_WARN("visibility/occ_max_prob must be in [0, 1); using 0.95.");
    visibility_occ_max_prob_ = 0.95;
  }
  if (!std::isfinite(visibility_unknown_prob_) ||
      visibility_unknown_prob_ < 0.0 || visibility_unknown_prob_ > 1.0) {
    ROS_WARN("visibility/unknown_prob must be in [0, 1]; using 0.0.");
    visibility_unknown_prob_ = 0.0;
  }
  if (!std::isfinite(visibility_eps_) || visibility_eps_ <= 0.0) {
    ROS_WARN("visibility/eps must be positive; using 1e-6.");
    visibility_eps_ = 1e-6;
  }
  if (visibility_model_ == "ray" && !mapPtr_) {
    ROS_WARN("Ray visibility is enabled without an occupancy map; its cost will be disabled.");
  }
}
...
void TrajOpt::addTimeCost(double& cost) {
  ...
  for (int i = 0; i < M; ++i) {
    ...
    if (landing_) {
      ...
      bool visibility_cost_active = false;
      if (visibility_model_ == "legacy") {
        visibility_cost_active =
            grad_cost_visibility(pos, target_p, tracking_visible_ps_[i],
                                 tracking_thetas_[i], grad_tmp, cost_tmp);
      } else if (visibility_model_ == "ray") {
        visibility_cost_active =
            grad_cost_visibility_ray(pos, target_p, grad_tmp, cost_tmp);
      }
      if (visibility_cost_active) {
        gradViolaPc = beta0 * grad_tmp.transpose();
        cost += rho * step * cost_tmp;
        jerkOpt_.gdC.block<6, 3>(piece * 6, 0) += rho * step * gradViolaPc;
        if (piece > 0) {
...
bool TrajOpt::evaluateRayOpticalDepth(const Eigen::Vector3d& p,
                                      const Eigen::Vector3d& target_p,
                                      double& tau,
                                      Eigen::Vector3d& grad_tau) const {
  tau = 0.0;
  grad_tau.setZero();
  if (!mapPtr_ || mapPtr_->resolution <= 0.0 || visibility_ray_samples_ <= 0) {
    return false;
  }

  const Eigen::Vector3d d = p - target_p;
  const double ray_length = d.norm();
  ...
  double sum_rho = 0.0;
  Eigen::Vector3d sum_s_grad_rho = Eigen::Vector3d::Zero();
  const double inv_samples = 1.0 / static_cast<double>(visibility_ray_samples_);

  for (int j = 0; j < visibility_ray_samples_; ++j) {
    const double s = (static_cast<double>(j) + 0.5) * inv_samples;
    const Eigen::Vector3d ray_sample = target_p + s * d;

    double occ_proxy;
    Eigen::Vector3d grad_occ;
    if (!mapPtr_->queryOccupancyProxy(ray_sample, visibility_unknown_prob_,
                                      occ_proxy, grad_occ)) {
      tau = 0.0;
      grad_tau.setZero();
      return false;
    ...
    const double q = visibility_occ_max_prob_ * occ_proxy;
    const double denominator = 1.0 - q + visibility_eps_;
    ...
    const double rho = -std::log(denominator) / mapPtr_->resolution;
    const Eigen::Vector3d grad_rho =
        visibility_occ_max_prob_ /
        (mapPtr_->resolution * denominator) * grad_occ;
    sum_rho += rho;
    sum_s_grad_rho += s * grad_rho;
  }

  tau = ray_length * inv_samples * sum_rho;
  grad_tau = d / ray_length * (inv_samples * sum_rho) +
             ray_length * inv_samples * sum_s_grad_rho;
  return std::isfinite(tau) && grad_tau.allFinite();
}

bool TrajOpt::grad_cost_visibility_ray(const Eigen::Vector3d& p,
                                       const Eigen::Vector3d& target_p,
                                       Eigen::Vector3d& gradp,
                                       double& costp) const {
  gradp.setZero();
  ...
  if (!evaluateRayOpticalDepth(p, target_p, tau, grad_tau)) {
    return false;
  }

  const double tau_safe = -std::log(visibility_T_safe_);
  const double violation = tau - tau_safe;
  if (violation > 0.0) {
    const double violation_squared = violation * violation;
    costp = rhoVisibilityRay_ * violation_squared * violation;
    gradp = rhoVisibilityRay_ * 3.0 * violation_squared * grad_tau;
  }

  if (visibility_debug_) {
    ROS_INFO_STREAM_THROTTLE(1.0, "ray visibility: tau=" << tau
                                                          << ", T=" << std::exp(-tau)
                                                          << ", cost=" << costp
                                                          << ", |grad_tau|=" << grad_tau.norm());
  }
  return violation > 0.0;
}

bool TrajOpt::checkVisibilityRayGradient(
  ...
  grad_analytic.setZero();
  grad_finite_difference.setZero();
  relative_error = 0.0;
  if (!std::isfinite(finite_difference_step) || finite_difference_step <= 0.0) {
    return false;
  }
  ...
  double cost;
  grad_cost_visibility_ray(p, target_p, grad_analytic, cost);
  for (int axis = 0; axis < 3; ++axis) {
    Eigen::Vector3d p_plus = p;
    Eigen::Vector3d p_minus = p;
    p_plus(axis) += finite_difference_step;
    p_minus(axis) -= finite_difference_step;

    Eigen::Vector3d unused_grad;
    double cost_plus, cost_minus;
    grad_cost_visibility_ray(p_plus, target_p, unused_grad, cost_plus);
    grad_cost_visibility_ray(p_minus, target_p, unused_grad, cost_minus);
    grad_finite_difference(axis) =
        (cost_plus - cost_minus) / (2.0 * finite_difference_step);
  }

  relative_error = (grad_analytic - grad_finite_difference).norm() /
                   std::max(1.0, grad_finite_difference.norm());
  ROS_INFO_STREAM("Ray visibility gradient check: analytic=["
                  << grad_analytic.transpose() << "], finite_difference=["
                  << grad_finite_difference.transpose() << "], relative_error="
                  << relative_error);
  return std::isfinite(relative_error) && grad_finite_difference.allFinite();
}
