...
struct OccGridMap {
 ...
 public:
  ...
  inline bool queryOccupancyProxy(const Eigen::Vector3d& p,
                                  const double p_unknown,
                                  double& occ_proxy,
                                  Eigen::Vector3d& grad_occ) const {
    occ_proxy = p_unknown;
    grad_occ.setZero();
    if (resolution <= 0.0 || infocc.data.empty() || !p.allFinite()) {
      return false;
    }

    // Voxel values are located at centers, hence the half-voxel shift.
    const Eigen::Vector3d grid_p = p / resolution - Eigen::Vector3d::Constant(0.5);
    const Eigen::Vector3i base = grid_p.array().floor().cast<int>();
    const Eigen::Vector3d frac = grid_p - base.cast<double>();

    occ_proxy = 0.0;
    for (int dx = 0; dx <= 1; ++dx) {
      const double wx = dx == 0 ? 1.0 - frac.x() : frac.x();
      const double dwx = dx == 0 ? -1.0 : 1.0;
      for (int dy = 0; dy <= 1; ++dy) {
        const double wy = dy == 0 ? 1.0 - frac.y() : frac.y();
        const double dwy = dy == 0 ? -1.0 : 1.0;
        for (int dz = 0; dz <= 1; ++dz) {
          const double wz = dz == 0 ? 1.0 - frac.z() : frac.z();
          const double dwz = dz == 0 ? -1.0 : 1.0;
          const Eigen::Vector3i id = base + Eigen::Vector3i(dx, dy, dz);

          double voxel_proxy = p_unknown;
          if (isInMap(id)) {
            const int8_t state = infocc.atId(id);
            voxel_proxy = state == 1 ? 1.0 : (state == -1 ? 0.0 : p_unknown);
          }

          occ_proxy += voxel_proxy * wx * wy * wz;
          grad_occ.x() += voxel_proxy * dwx * wy * wz;
          grad_occ.y() += voxel_proxy * wx * dwy * wz;
          grad_occ.z() += voxel_proxy * wx * wy * dwz;
        }
      }
    }
    grad_occ /= resolution;
